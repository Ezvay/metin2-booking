...
const { sendBookingNotification, handleInteraction, PRICES, EQ_PRICES, EQ_DEPOSIT } = require('../discord');

const HCAPTCHA_SECRET = process.env.HCAPTCHA_SECRET;
const HCAPTCHA_SITE_KEY = '638a1d80-4b9c-44d2-9f2e-e78c063a2580';

async function verifyHcaptcha(token) {
  if (!HCAPTCHA_SECRET) return true; // Skip in dev
  try {
...
router.post('/api/slots/:id/book', async (req, res) => {
  ...
  // If slot already has a package type set, must match
  if (slot.package_type && slot.package_type !== package_type) {
    return res.json({ success: false, error: `Ten slot jest juz zarezerwowany jako ${slot.package_type}. Wybierz ten sam typ.` });
  }
  ...
  // Determine max players from package type
  const maxPlayers = { solo: 1, duo: 2, trio: 3 }[package_type] || 1;

  // If first booking — set package type and max players
  ...
  // If solo — check not already full
...
router.post('/api/reviews', async (req, res) => {
  ...
  if (existing) return res.json({ success: false, error: 'Juz oceniono te rezerwacje.' });
...
router.post('/api/admin/slots/generate', requireAdmin, async (req, res) => {
  ...
  const [fromH, fromM] = time_from.split(':').map(Number);
  const [toH, toM] = time_to.split(':').map(Number);
  const fromTotal = fromH * 60 + fromM;
  const toTotal = toH * 60 + toM;

  if (fromTotal >= toTotal) return res.json({ success: false, error: 'Godzina konca musi byc pozniejsza niz poczatek.' });
...
router.get('/api/admin/stats', requireAdmin, async (req, res) => {
  ...
  const byPackage = await db.all2(`
    SELECT package_type, COUNT(*) as count FROM bookings
    WHERE status='done' GROUP BY package_type
  `);
  ...
  const eqStats = await db.all2(`
    ...
    WHERE status != 'cancelled' AND equipment_rental != 'none'
    GROUP BY equipment_rental
  `);
  res.json({
    ...
    totalReviews: avgRating.c,
    byPackage, byMonth: byMonth.reverse(), eqStats
  });
});

// ── ADMIN: ADD EXTRA MEMBER ───────────────────────────────────
router.post('/api/admin/slots/:id/add-member', requireAdmin, async (req, res) => {
  const { char_name, char_class, char_level, contact_discord, equipment_rental } = req.body;
  if (!char_name || !char_class || !char_level || !contact_discord) {
    return res.json({ success: false, error: 'Wypelnij wszystkie pola.' });
  }
  const slot = await db.get2('SELECT * FROM slots WHERE id=?', [req.params.id]);
  if (!slot) return res.json({ success: false, error: 'Slot nie istnieje.' });

  const token = crypto.randomBytes(14).toString('hex');
  await db.run2(`
    INSERT INTO bookings (slot_id, char_name, char_class, char_level, contact_discord, package_type, equipment_rental, status_token, status)
    VALUES (?,?,?,?,?,?,?,?,'active')
  `, [req.params.id, char_name, char_class, char_level, contact_discord, slot.package_type || 'solo', equipment_rental || 'none', token]);
